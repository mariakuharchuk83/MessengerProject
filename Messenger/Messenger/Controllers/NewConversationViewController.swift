...
class NewConversationViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        noResultsLable.frame(forAlignmentRect: CGRect(x: view.width/4, y: (view.height-200)/2, width: view.width/2, height: 100))
    }
...
extension NewConversationViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //start conversation
        let targerUsersData = results[indexPath.row]
        
        dismiss(animated: true) {[weak self] in
            self?.completion?(targerUsersData)
        }
    }
...
extension NewConversationViewController: UISearchBarDelegate{
    ...
    func filterUsers(with term: String) {
        //update the UI: show results of show no results label
        guard let currentUserEmail = UserDefaults.standard.value(forKey: "email") as? String, hasFetched else {
            return
        }
        
        let safeEmail = currentUserEmail.parseToSafeEmail()
